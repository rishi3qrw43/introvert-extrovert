...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      P('Every preprocessing step runs inside a scikit-learn [18] pipeline, so imputation and scaling are fitted on training data only. Fitting them on the full dataset first would be a small version of the leakage this paper measures.'),
      ...
      BULLET([t('Class structure. ', { bold: true }), t('Three classes (introvert, extrovert, neither) against two classes, with the neither group removed. This one changes the prediction task and the population being classified, not only the order of the steps.')]),
      BULLET([t('Resampling order. ', { bold: true }), t('SMOTE [16] applied to the entire dataset before cross-validation, against SMOTE applied inside each training fold through an imbalanced-learn [20] pipeline. SMOTE builds a new minority-class row by taking a real minority row, picking one of its five closest minority neighbors, and placing a new point at a random spot on the line between them. Five neighbors is the default and is used in both arms, so the only thing that differs is when the new rows are made.')]),
      ...
      P('Choosing features on the whole dataset can only leak something if the choice itself carries information. If the questions in a dataset all measure close to the same thing, then almost any subset of them carries almost the same information, and it stops mattering whether that subset was picked using the test rows or without them. The measurements below suggest that is the situation here.'),
      P('Table 3 reports how strongly the items within each set move together. The measure is the absolute correlation between every pair of items in a set, where 0 means two questions tell you unrelated things about a person and 1 means they tell you the same thing twice. A set with a high average is one where the questions are close to interchangeable.'),
      table3,
